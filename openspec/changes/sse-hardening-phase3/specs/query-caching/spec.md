...
- A metric SHALL track cache hit ratio (`ontology.cache.sparql.hits` and `ontology.cache.sparql.misses`)
...
- **THEN** `ontology_cache_sparql_hits_total` and `ontology_cache_sparql_misses_total` SHALL be available
